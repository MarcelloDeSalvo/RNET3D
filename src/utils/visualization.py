...
def plot_data_axis(data, slice_id=None, axis=2, rotation=0):
    ...
    for i, modality in enumerate(data.keys()):
        ...
        if modality == 'lesion_mask':
            # Use a different colormap for the mask (e.g., 'jet')
            plt.imshow(np.rot90(np.take(data[modality], slice_id, axis=axis), k=rotation), cmap='jet')
        else:
            # Use the 'gray' colormap for other modalities
            plt.imshow(np.rot90(np.take(data[modality], slice_id, axis=axis), k=rotation), cmap='gray')
...
def plot_overlap(orig, mask, axis=2, alpha=0.5, slice_id=0, rotation=0):
    ...
    plt.imshow(np.rot90(np.take(mask, slice_id, axis=axis), k=-1), cmap='jet', alpha=alpha)
    plt.show()
...
def explore_3D_array_comparison(arr_before: np.ndarray, arr_after: np.ndarray, cmap: str = 'gray', axis: int = 0):
  """
...
def explore_3D_overlay(arr_before: np.ndarray, mask: np.ndarray, cmap: str = 'gray', axis: int = 0, alpha: float = 0.5):
...
def explore_3D_predictions(arr_before: np.ndarray, ground_truth: np.ndarray, pred: np.ndarray, cmap: str = 'gray', axis: int = 0, alpha: float = 0.5):
...
# explore 3D array comparison with a slider to compare original overlayed with ground truth mask
def explore_3D_array_comparison_overlay(arr_data: np.ndarray, arr_mask: np.ndarray, cmap: str = 'gray', axis: int = 0, alpha: float = 0.5):
...
def plot_predictions(ids, index, generator, model, alpha = 0.4):
  ...
  explore_3D_predictions(np.rot90(processed_image[0,:,:,:,0], k=-1), np.rot90(gt[0,:,:,:], k=-1), np.rot90(p[0,:,:,:], k=-1), alpha=alpha, axis=2)
...
def plot_domain_adaptation(ids, index, generator, model, alpha = 0.4):
  ...
  print( 'Real condition: ' + str(condition[0]) + ' Predicted condition: ' + str(predictions[2][0]) )
  ...
  explore_3D_predictions(np.rot90(processed_image[0,:,:,:,0], k=-1), np.rot90(regions[0,:,:,:], k=-1), np.rot90(predictions[0][0,:,:,:], k=-1), alpha=alpha, axis=2)
  explore_3D_predictions(np.rot90(processed_image[0,:,:,:,0], k=-1), np.rot90(mask[0,:,:,:,0], k=-1), np.rot90(predictions[1][0,:,:,:,0], k=-1), alpha=alpha, axis=2)
