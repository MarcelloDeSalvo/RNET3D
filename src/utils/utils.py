...
def save_excel_table(
    ...
    save_folder,
    name_mapping,
    pred_roi_name="unet_prediction.nii.gz",
    pred_brain_name="brain_mask.nii.gz",
    file_name="labels_volumes.xlsx",
    ...
    - name_mapping (list): list of dictionaries mapping class values to names
    ...
    def get_subjects_list(modality_folder):
        ...
        return [
            subject
            for subject in os.listdir(modality_folder)
            if os.path.isdir(os.path.join(modality_folder, subject))
        ]
    ...
    def process_subject(case, volumes):
        ...
        if postfix_mode:
            y_pred_path = os.path.join(
                modality_folder, case, sub_folder, case + pred_roi_name
            )
            print(y_pred_path)
            brain_mask_path = os.path.join(
                modality_folder, case, sub_folder, case + pred_brain_name
            )
        else:
            y_pred_path = os.path.join(modality_folder, case, sub_folder, pred_roi_name)
            brain_mask_path = os.path.join(
                modality_folder, case, sub_folder, pred_brain_name
            )
        ...
        if brain_mask is None:
            ...
            brain_volume = estimate_volume(brain_mask).get(
                1, 0
            )  # Assuming label 1 is the brain mask
    ...
    num_classes = len(name_mapping)
    volumes = {
        name_mapping[i].get("name", f"Class_{i}"): [] for i in range(num_classes)
    }
