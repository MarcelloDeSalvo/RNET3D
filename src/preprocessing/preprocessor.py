...
class PreprocessingStep(abc.ABC):
    ...
    def apply(self, imgs=None, masks=None):
        '''
        Apply the function to the images and the masks based on their type.
        '''
        if np.random.rand() > self.probability: return self.return_processed(imgs, masks)
...
class Preprocessor:
    ...
    def preprocess(self, imgs=None, masks=None):
        if self.verbose: print("Preprocessing steps{\n")

        for step in self.steps:
            if self.verbose: print(step.get_configuration())
            if imgs is not None:
                if masks is not None:
                    imgs, masks = step.apply(imgs, masks)
                else:
                    imgs = step.apply(imgs=imgs)
            else:
                masks = step.apply(masks=masks)
    ...
    def deprocess(self, prediction, original_image, mapping, save_path=None, verbose=False, reference_resolution=(0.1,0.1,0.1), flip_axis=None, scale_factor=None):
        '''
        Reverses the preprocessing steps given the original image as reference.
        '''
        if scale_factor is not None:
            prediction = Scale(scale_factor, verbose=verbose).execute(prediction, is_mask=True)

        reoriented_prediction = Reorient(original_image, verbose=verbose).execute(prediction)
        corrected_original = CorrectX10(reference_resolution, verbose=verbose).execute(original_image)
        corrected_resolution = corrected_original.header.get_zooms()
        resampled_prediction = Resample(corrected_resolution, verbose=verbose).execute(reoriented_prediction, is_mask=True)
        matched_prediction = PadderCutter(original_image.shape, verbose=verbose).execute(resampled_prediction)
        remapped_prediction = MapLabels(mapping=mapping, verbose=verbose).execute(matched_prediction, is_mask=True)

        if flip_axis is not None:
            remapped_prediction = Flip(flip_axis, verbose=verbose).execute(remapped_prediction)
            
        if save_path is not None:
            SaveNifti(save_path, reference=original_image, verbose=verbose).execute(remapped_prediction)
...
class Resample( PreprocessingStep ):
    ...
    def execute(self, image, is_mask=False):
        ...
        affine = nib.affines.rescale_affine(image.affine, shape=image.shape, zooms=self.target_resolution)

        voxel_dims = image.header.get_zooms()
        if self.verbose: print("\t|- Original voxel dimensions: " , voxel_dims)

        # if they are close to the target resolution, skip resampling
        if np.allclose(voxel_dims, self.target_resolution, atol=1e-2):
            if self.verbose: print("\t|- Voxel dimensions are already close to the target resolution.")
            return image
    
        scale_factors =  [current_dim / target_dim for current_dim, target_dim in zip(voxel_dims, self.target_resolution)]
        if self.verbose: print("\t|- Scale factors: " , scale_factors)

        _order = 0 if is_mask else self.interpolation
        _mode = 'nearest' if is_mask else 'constant'
        resampled_data = zoom(image.get_fdata(), scale_factors, order=_order, mode=_mode)

        if self.verbose: print("\t|- Resampled image shape: " , resampled_data.shape)
...
class CorrectX10( PreprocessingStep ):
    ...
    def __init__(self, reference_resolution=(0.1,0.1,0.1), threshold=5, verbose=False):
        super().__init__( verbose)
    ...
    def execute(self, image, is_mask=False):
        ...
        detected_factors = [np.round(dim / ref_dim) for dim, ref_dim in zip(voxel_dims, self.reference_resolution)]
        if self.verbose: print("\t|- Detected factors: " , detected_factors)

        # Divide by a factor of 10 if one of the factors is close to a factor of 10
        if np.any([np.abs(factor - 10) < self.threshold for factor in detected_factors]):
            voxel_dims = [dim / 10 for dim in voxel_dims]

        if self.verbose: print("\t|- Corrected factors: " , voxel_dims)
        new_affine = nib.affines.rescale_affine(image.affine, shape=image.shape, zooms=voxel_dims)

        return nib.Nifti1Image(image.get_fdata(), affine=new_affine, header=image.header, dtype=np.float64)
...
class MapLabels ( PreprocessingStep ):
    ...
    def __init__(self, labels=None, mapping=None, reverse=None, verbose=False):
        ...
        if mapping is None:
            self.mapping = {old_label: new_label for new_label, old_label in enumerate(labels)}
        elif reverse:
            if self.verbose: print("\t|- Reversing mapping...")
            self.mapping = {new_label: old_label for old_label, new_label in mapping.items()}
        else:
            self.mapping = mapping

        if self.verbose: print("\t|- Mapping labels: ", self.mapping)
    ...
    def execute(self, image, is_mask=False):
        ...
        '''
        if not is_mask: return image

        data = image.get_fdata()
        if self.verbose: print("\t|- Unique labels: ", np.unique(data))
        if self.verbose: print("\t|- Data type: ", data.dtype)

        tmp_data = np.zeros(data.shape)
        for old_label, new_label in self.mapping.items():
            tmp_data[data == old_label] = new_label
            if self.verbose: print(f"\t|- Mapping label {old_label} to {new_label}")
        if self.verbose: print("\t|- Unique labels: ", np.unique(tmp_data))
...
class Reorient ( PreprocessingStep ):
    ...
    def execute(self, image, is_mask=False):
        ...
        if self.verbose: print("\t|- Old orientation: ", nib.orientations.ornt2axcodes(old_ornt))
        ...
        if self.verbose: print("\t|- New orientation: ", copied_codes)

        transform = ornt_transform(old_ornt, new_ornt)

        image = image.as_reoriented(transform)

        return nib.Nifti1Image(image.get_fdata(), image.affine, image.header, dtype=np.float64)
...
class N4BiasFieldCorrection ( PreprocessingStep ):
    ...
    def __init__(self, tolerance=0, iters=[50,50,50,50], spline_parameters=[1,1,1], spline_order=3, rescale_intensities=False, masking=False, verbose=False):
        super().__init__(verbose)
    ...
    def execute(self, image, is_mask=False):
        ...
        return nib.Nifti1Image(ants_image.numpy(), image.affine, image.header, dtype=np.float64)
...
class Padder( PreprocessingStep ):
    ...
    def iterate(self, imgs=None, masks=None):
        ...
        if imgs:
            img_shape = imgs[0].get_fdata().shape if isinstance(imgs, list) else imgs.get_fdata().shape
            padding_values = self.generate_padding_values(img_shape, self.dim)

        # Pad images and masks using the generated padding values
...
class PadderCutter( PreprocessingStep ):
    ...
    def execute(self, image, is_mask=False):
        ...
        '''
        _diff_x = self.img_size[0] - image.shape[0]
        _diff_y = self.img_size[1] - image.shape[1]
        _diff_z = self.img_size[2] - image.shape[2]
        
        data = image.get_fdata()

        # Pad or cut the image along each dimension
        if _diff_x > 0:
            data = np.pad(data, ((0, _diff_x), (0, 0), (0, 0)), mode='constant')
        elif _diff_x < 0:
            data = data[:self.img_size[0], :, :]

        if _diff_y > 0:
            data = np.pad(data, ((0, 0), (0, _diff_y), (0, 0)), mode='constant')
        elif _diff_y < 0:
            data = data[:, :self.img_size[1], :]

        if _diff_z > 0:
            data = np.pad(data, ((0, 0), (0, 0), (0, _diff_z)), mode='constant')
        elif _diff_z < 0:
...
class SaveNifti( PreprocessingStep ):
    ...
    def execute(self, image, is_mask=False):
        ...
        '''
        if self.verbose: print(f"\t|- Saving image to {self.path}")
        if self.verbose: print(f"\t|- Image type: {type(image)}")

        if self.reference is None:
            if self.verbose: print(f"\t|- Cloning metadata from the original image.")
            image = nib.Nifti1Image(image.get_fdata(), image.affine, image.header, dtype=np.float64)
        nib.save(image, self.path)
        if self.verbose: print(f"\t|- Image saved to {self.path}")
...
class Noise ( PreprocessingStep ):
    ...
    def execute(self, image, is_mask=False):
        ...
        '''
        if is_mask == True: return image
        image_data = image.get_fdata()
        std = np.random.uniform(self.noise_range[0], self.noise_range[1])
        noise = np.random.normal(self.mean, std, image.shape)
        return nib.Nifti1Image(image_data + noise, affine=image.affine, header=image.header, dtype=np.float64)
...
class GaussianBlur(PreprocessingStep):
    ...
    def execute(self, img, is_mask=False):
        ...
        '''
        if is_mask == True: return img
        img_data = img.get_fdata()
        sigmas = np.random.uniform(self.gaussian_blur_range[0], self.gaussian_blur_range[1], 3)
        return nib.Nifti1Image(ndimage.gaussian_filter(img_data, sigmas), affine=img.affine, header=img.header, dtype=np.float64)
...
class RandomAffine ( PreprocessingStep ):
    ...
    Attributes:
        ...
        geometric (bool): If the augmentation is geometric or not, to know if it should be applied to the mask.
    '''
    def __init__(self, scale_range=None, rotation_range=None, probability=0.5, interpolation=sitk.sitkNearestNeighbor, background_label=0):
        super().__init__(probability=probability)
    ...
    def iterate(self, imgs=None, masks=None):
        ...
        angles = np.random.uniform(self.rotation_range[0], self.rotation_range[1], 3) if self.rotation_range is not None else None
        scale_factor = np.random.uniform(self.scale_range[0], self.scale_range[1]) if self.scale_range is not None else None
    ...
    def execute(self, img, angles=None, scale_factor=None, is_mask=False):
        ...
        if angles is not None:
            rotation_transform = sitk.Euler3DTransform(rotation_center, np.radians(angles[0]), np.radians(angles[1]), np.radians(angles[2]))
            affine_transform = sitk.CompositeTransform([affine_transform, rotation_transform])

        if scale_factor is not None:
            scale_transform = sitk.ScaleTransform(3, [scale_factor, scale_factor, scale_factor])
            scale_transform.SetCenter(rotation_center)
            affine_transform = sitk.CompositeTransform([affine_transform, scale_transform])

        if is_mask:
            transformed_img = sitk.Resample(sitk.GetImageFromArray(data), affine_transform, sitk.sitkNearestNeighbor, self.background_label, sitk.sitkFloat64)
        else:
            transformed_img = sitk.Resample(sitk.GetImageFromArray(data), affine_transform, self.interpolation, 0.0, sitk.sitkFloat64)

        return nib.Nifti1Image(sitk.GetArrayFromImage(transformed_img), affine=img.affine, header=img.header, dtype=np.float64)
...
class Flip ( PreprocessingStep ):
    ...
    def execute(self, img, random_axis):
        img_data = img.get_fdata()
        return nib.Nifti1Image(np.flip(img_data, axis=random_axis), affine=img.affine, header=img.header, dtype=np.float64)
...
class RandomCropping( PreprocessingStep ):
    ...
    def get_center_cropping_coordinates(self, img):
        ...
        '''
        center_x, center_y, center_z = img.shape[0] / 2, img.shape[1] / 2, img.shape[2] / 2

        upper_x, upper_y, upper_z = img.shape[0] - self.dim[0] // 2, img.shape[1] - self.dim[1] // 2, img.shape[2] - self.dim[2] // 2
    ...
    def iterate(self, imgs=None, masks=None):
        ...
        '''
        x, y, z = self.select_cropping_coordinates(imgs[0] if isinstance(imgs, list) else imgs)
    ...
    def execute(self, img, x, y, z):
        ...
        '''
        img_data = img.get_fdata()
        img_data = img_data[x - self.dim[0] // 2:x + self.dim[0] // 2, y - self.dim[1] // 2:y + self.dim[1] // 2, z - self.dim[2] // 2:z + self.dim[2] // 2]
